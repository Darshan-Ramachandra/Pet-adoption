...
const MyAddedPets = () => {
  ...
  const setAdoptedStatus = (_id, adopted) => {
    console.log('Attempting to set adopted status:', { _id, adopted });
    fetch(`${baseUrl}/pets/${_id}`, {
      method: 'PATCH',
      headers: {
      ...
      .then(response => {
        if (!response.ok) {
          alert('Failed to update pet status. Server returned ' + response.status);
          throw new Error('Failed to update pet status');
        }
        return response.json();
      })
      .then(data => {
        console.log('Pet status updated:', data);
        // Re-fetch pets from backend to ensure UI is in sync
        fetch(`${baseUrl}/pets`)
          .then(response => response.json())
        ...
        <td>
          ...
          {!pet.adopted && (
            <button
              ...
              onClick={() => setAdoptedStatus(pet._id, true)}
            >
              Mark as Adopted
            </button>
          )}
