...
const PetListing = () => {
  ...
  const filterPetsByCategory = () => {
    // If no category is selected, show all pets that are not adopted
    if (!selectedCategory) {
      setFilteredPets(pets.filter(pet => !pet.adopted));
    } else {
      // Filter pets based on the selected category and not adopted
      const filtered = pets.filter((pet) => pet.category === selectedCategory && !pet.adopted);
